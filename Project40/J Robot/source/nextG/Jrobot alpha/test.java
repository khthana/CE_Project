...
public class test extends java.applet.Applet implements Runnable {
  ...
  public void init() {
    ...
    dataCenter = new DataCenter(new Point(MINX,MINY),
	  ...
	  GameOver = false;
	  GameDraw = false;
	  sleep_time = Integer.parseInt(getParameter("sleep_time"));
	  fireSize = new Dimension(images[0][FIRE].getWidth(this),
	                           images[0][FIRE].getHeight(this));
	  cStep = 5;
	  rgb = 0;
	  currentColor = RED;
	  rColor = new Color[8];
	  ...
	  while (i < NUMROBOT) {
	    ...
	    robot[i] = new Model(new Point(MINX + imageWidth/2,MINY + imageHeight/2),
	                         new Point(screenWidth - imageWidth/2,
		                       ...
		                       imageWidth,imageHeight,sleep_time * 2,dataCenter,i);
	    b = false;
      if (i != 0) {
  ...
  public void paint(Graphics g) {
	  if (GameOver) {
	    ...
	    // -------------------------------------------------------------------
	    for (int i = 0; i < NUMROBOT; i++) {
		    if (robot[i] != null && robot[i].fired()) {
		      Point fPoint;
		      fPoint = robot[i].getfPosition();
		      bufferGraphics.drawImage(images[i][FIRE],fPoint.x - fireSize.width/2,
		                               fPoint.y - fireSize.height/2,this);
		    }
	    }
	    ...
	    for (int i = 0; i < NUMROBOT; i++) {
	  	  if (robot[i] != null && robot[i].fired()) {
		      Point fp1,fp2;
	  	    fp1 = robot[i].getfPosition();
          fp1 = new Point(fp1.x - fireSize.width/2,fp1.y - fireSize.height/2);
  		    fp2 = new Point(fp1.x + fireSize.width,fp1.y + fireSize.height);
	    ...
	    for (int i = 0; i < NUMROBOT; i++) {
            ...
            hit[i] = false;
		      ...
		      bufferGraphics.drawLine(p.x,p.y,
                           p.x + (int)(GUNLENGTH * cosine(rDir)),
			                     p.y + (int)(GUNLENGTH * sine(rDir)));
		    }
	    }
      ...
      for (int i = 0; i < NUMROBOT; i++) {
	    ...
	    // -------------------------------------------------------------------
	    int space = (screenHeight - BASEY) / NUMROBOT;
	    for (int k = 0; k < NUMROBOT; k++) {
		    ...
		    if (!dataCenter.getDieFlag(k)) {
  		    ...
  		    scanDir = robot[k].getScanDir();
          scanArc = robot[k].getScanArc();
  		    info = robot[k].getInfo();
		    } else {
		      tmpCrp = new Point(0,0);
          scanDir = 0;
          scanArc = 0;
		    }

		    bufferGraphics.setColor(rColor[k]);
		    bufferGraphics.drawString("Rx = " + tmpCrp.x,BASEX,
                                     BASEY + 28 + (k * space));
		    bufferGraphics.drawString("Ry = " + tmpCrp.y,75,
                                     BASEY + 28 + (k * space));
        bufferGraphics.drawString("Sd = " + scanDir,BASEX,
                                     BASEY + 40 + (k * space));
        bufferGraphics.drawString("Sa = " + scanArc,75,
                                     BASEY + 40 + (k * space));
//		  bufferGraphics.drawString("Scan at " + scanDir,BASEX,
//                                   BASEY + 40 + (k * space));
//	  	bufferGraphics.drawString("degree",87,BASEY + 40 + (k * space));
//		  bufferGraphics.drawString("Found = " + sTable[0],BASEX,
//                                   BASEY + 52 + (k * space));
		    bufferGraphics.drawString("Ds = " + info[0],BASEX,
                                     BASEY + 52 + (k * space));
  		  bufferGraphics.drawString("Dr = " + info[1],75,
                                     BASEY + 52 + (k * space));
	    }
	  } // End if (GameOver) else ...
