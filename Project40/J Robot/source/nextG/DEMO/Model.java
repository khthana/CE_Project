...
// ============================================================================
  public Model(int idx,DataCenter dc,int t) {
	  dataCenter = dc;
	  index = idx;
	  hitpoint = MAXHP;
    imageSize = dataCenter.getImageSize();
    minPoint = dataCenter.getMinP();
    minPoint = new Point(minPoint.x + imageSize.width / 2,
                         minPoint.y + imageSize.height / 2);
    maxPoint = dataCenter.getMaxP();
    maxPoint = new Point(maxPoint.x - imageSize.width / 2,
  ...
  public synchronized void fire(int dir) {
    ...
    while (fid < NUMFIRE && !done) {
	    if ((fire[fid] == null || fire[fid].isDead()) && fireWait[fid] == 0) {
