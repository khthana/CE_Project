...
public class Simulator extends Applet implements Runnable {
  ...
  public void init() {
    ...
    robot_type = new int[4];
    num_robot = 0;
    int count = 1;
    int parameter;
    while(count <= 4) {
      parameter = Integer.parseInt(getParameter("TYPE OF ROBOT" + count));
      if (parameter != 0) {
        robot_type[num_robot] = parameter;
        num_robot++;
      }
      count++;
    }
    ...
    tracker.addImage(bgImage,0);
	  images = new Image[num_robot][NUMPIC];
    ...
    dataCenter = new DataCenter(new Point(MINX,MINY),
	  ...
	  robot = new Model[num_robot];
	  hit = new boolean[num_robot];
	  int i = 0;
	  boolean b;
    ...
    //
      ...
      switch (robot_type[i]) {
        case 1:
	      robot[i] = new CycleX(i,dataCenter,SLEEPING * 2);
	      break;
        case 2:
        robot[i] = new RandoZ(i,dataCenter,SLEEPING * 2);
        break;
        case 3:
        robot[i] = new StandNShoot(i,dataCenter,SLEEPING * 2);
        break;
        case 4:
        robot[i] = new WalkAround(i,dataCenter,SLEEPING * 2);
      }
      b = false;
      if (i != 0) {
  ...
  public void paint(Graphics g) {
      ...
      for (int i = 0; i < num_robot; i++) {
	    ...
	    // -------------------------------------------------------------------
	    int space = (screenHeight - BASEY - 36) / num_robot;
