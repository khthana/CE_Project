function [pwd,count_file]=seg_image2(pwd,image_file);
more off;
echo off;
clc;
...
for a_1=1:maxs
   ...
   for pixel_x=1:info.Width
      have = all(im(:,pixel_x));
      if have==0
         for pixel_y=1:ih
            if im(pixel_y,pixel_x)==0
               im(pixel_y,pixel_x) = 1;
               [im,bound_y,bound_x] = segment...
               (im,pixel_y,pixel_x,1,info.Width,1,ih);
