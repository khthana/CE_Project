...
type
  ...
  public
    ...
    procedure Playwave(name:string);
  end;
...
begin
  if Audio<>nil then
  begin
    s := Audio.Position div Audio.Format^.nBlockAlign div Audio.Format^.nSamplesPerSec;
...
procedure TMainForm.Playwave(name: string);
var
  WaveFormat: TWaveFormatEx;
begin
    FileCloseClick(nil);
  ...
  try
    Caption := Application.Title + ' - ' + ExtractFileName(Name);
    if not(DXSound.Initialized) then
    DXSound.Initialize;
