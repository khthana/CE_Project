...
{	protected int[] key_schedule = new int[32];
	...
	Util u = new Util();
	// generate DES Key n byte
	public byte[] genKey(int n)
	{	byte[] DES = new byte[n];
		for (int i=0 ;i<n ;i++ )
		{	DES[i]=byteRandom();
			try{Thread.sleep(20);}catch (InterruptedException e){System.err.println("InterruptedException "+e);}
		}	
		return DES;
	}
//===============Padding Message ==========================================
// if message length %(modulus) with key size not equal zero then padding message 
//  if msg.length % keysize != 0    then padding message => add zero 

	public byte[] paddingMessage(byte[] msg ,int keysize)
	{	byte[] msg_tmp;
	...
	public byte[] dePaddingMessage(byte[] msg_padded)
	{	byte [] len =new byte [2];
		len[0] = msg_padded [0];
		len[1] = msg_padded [1];
	...
	{	byte[] enc = new byte[in.length];
		...
		{    iv0 ^= ((in[i] & 0xff) | ((in[i + 1] & 0xff) << 8) |
				((in[i + 2] & 0xff) << 16) | ((in[i + 3] & 0xff) << 24));
			iv1 ^= ((in[i + 4] & 0xff) | ((in[i + 5] & 0xff) << 8) |
				((in[i + 6] & 0xff) << 16) | ((in[i + 7] & 0xff) << 24));
			encrypt(iv0, iv1, out);
			iv0 = out[0];
			iv1 = out[1];
			enc[i]   = (byte)( iv0         & 0xff);
			enc[i+1] = (byte)((iv0 >>> 8 ) & 0xff);
			enc[i+2] = (byte)((iv0 >>> 16) & 0xff);
			enc[i+3] = (byte)((iv0 >>> 24) & 0xff);
			enc[i+4] = (byte)( iv1         & 0xff);
			enc[i+5] = (byte)((iv1 >>> 8 ) & 0xff);
			enc[i+6] = (byte)((iv1 >>> 16) & 0xff);
			enc[i+7] = (byte)((iv1 >>> 24) & 0xff);
		}
		IV0 = iv0;
		IV1 = iv1;
		return enc;
	...
	{	byte[] dec = new byte[in.length];
		...
		int d1;
		for(int i = 0; i < in.length; i += 8) 
		{	d0 = ((in[i] & 0xff) | ((in[i + 1] & 0xff) << 8) | 
				((in[i + 2] & 0xff) << 16) | ((in[i + 3] & 0xff) << 24));
			d1 = ((in[i + 4] & 0xff) | ((in[i + 5] & 0xff) << 8) | 
				((in[i + 6] & 0xff) << 16) | ((in[i + 7] & 0xff) << 24));
			decrypt(d0, d1, out);
			iv0 ^= out[0];
			iv1 ^= out[1];
			dec[i]   = (byte)( iv0         & 0xff);
			dec[i+1] = (byte)((iv0 >>> 8 ) & 0xff);
			dec[i+2] = (byte)((iv0 >>> 16) & 0xff);
			dec[i+3] = (byte)((iv0 >>> 24) & 0xff);
			dec[i+4] = (byte)( iv1         & 0xff);
			dec[i+5] = (byte)((iv1 >>> 8 ) & 0xff);
			dec[i+6] = (byte)((iv1 >>> 16) & 0xff);
			dec[i+7] = (byte)((iv1 >>> 24) & 0xff);
			iv0 = d0;
			iv1 = d1;
		}
		IV0 = iv0;
