...
//------------------------1---------------------------------
       ...
       pstmt = con.prepareStatement("SELECT distinct car_menufacturer FROM car WHERE model="+"'"+model+"'");  
...
//------------------------3---------------------------------    
      ...
      pstmt = con.prepareStatement("SELECT distinct year FROM car WHERE model="+"'"+model+"' ORDER BY year ASC");
      rs = pstmt.executeQuery();

	out.println("<SELECT NAME=\"year\" SIZE=\"1\" onchange=\"doReload111(this.form)\">");
