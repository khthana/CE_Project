...
public class Form1 extends HttpServlet {
  ...
  public void service(HttpServletRequest req, HttpServletResponse res)
    ...
    if (values!=null) { sp_namex = values[0]; }   
	...
	out.println("function doReload11(entry) {");
	...
	out.println("url = \"http://"+connect.hostname+"/servlets/Form11?master=\"+master+\"&menufacture=\"+menufacture+\"&model=\"+model+\"&year=\"+year+\"&sp_namex=\"+sp_namex+\"&lastpage=\"+lastpage;");
	out.println("if (model!=\"\")"); 
	out.println("window.location.href = url;");
	out.println("}");

	out.println("function doReload12(entry) {");
	out.println("var current1,current2,current3;");		
	out.println("var menufacture,model,year,master,url;");
	out.println("current1 = entry.menufacture.selectedIndex;");
	out.println("menufacture = entry.menufacture.options[current1].value;");
...
//------------------------2---------------------------------    
      ...
      pstmt = con.prepareStatement("SELECT distinct model FROM car WHERE car_menufacturer="+"'"+menufacture+"' ORDER BY model ASC");
      rs = pstmt.executeQuery();

	out.println("<SELECT NAME=\"model\" SIZE=\"1\" onchange=\"doReload11(this.form)\">");
...
//------------------------3---------------------------------    
      ...
      pstmt = con.prepareStatement("SELECT distinct year FROM car WHERE car_menufacturer="+"'"+menufacture+"' ORDER BY year ASC");
      rs = pstmt.executeQuery();

	out.println("<SELECT NAME=\"year\" SIZE=\"1\" onchange=\"doReload12(this.form)\">");
	out.println("<OPTION SELECTED VALUE=\"\">-------</OPTION>");
