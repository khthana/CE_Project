  ...
  public void service(HttpServletRequest req, HttpServletResponse res)
    ...
    //out.println(sp_namex);
   	...
   	out.println("<HTML><HEAD><TITLE>Choose Car</TITLE></HEAD>");
        out.println("<body background=\"/image/41.gif\">");        		out.println("<center><img src=\"http://"+connect.hostname+"/image/bar2.gif\"></center>");      
   	out.println("<br>");
        
        out.println("<SCRIPT language=JavaScript>");

	out.println("function doReload1(entry) {");
	...
	out.println("url = \"http://"+connect.hostname+"/servlets/Form1?master=\"+master+\"&menufacture=\"+menufacture+\"&model=\"+model+\"&year=\"+year+\"&sp_namex=\"+sp_namex+\"&lastpage=\"+lastpage;");
	out.println("if (menufacture!=\"\")"); 
	out.println("window.location.href = url;");
	out.println("}");

	out.println("function doReload2(entry) {");
	out.println("var current1,current2,current3;");		
	out.println("var menufacture,model,year,master,url;");
	out.println("current1 = entry.menufacture.selectedIndex;");
	out.println("menufacture = entry.menufacture.options[current1].value;");
	...
	out.println("year = entry.year.options[current3].value;");
        ...
        out.println("sp_namex=entry.sp_namex.value;");
        out.println("lastpage=entry.lastpage.value;");        
	out.println("url = \"http://"+connect.hostname+"/servlets/Form2?master=\"+master+\"&menufacture=\"+menufacture+\"&model=\"+model+\"&year=\"+year+\"&sp_namex=\"+sp_namex+\"&lastpage=\"+lastpage;");
	out.println("if (model!=\"\")"); 
	out.println("window.location.href = url;");
	out.println("}");

	out.println("function doReload3(entry) {");
	out.println("var current1,current2,current3;");		
	out.println("var menufacture,model,year,master,url;");
	out.println("current1 = entry.menufacture.selectedIndex;");
	out.println("menufacture = entry.menufacture.options[current1].value;");
	...
	out.println("year = entry.year.options[current3].value;");
        ...
        out.println("sp_namex=entry.sp_namex.value;");
        out.println("lastpage=entry.lastpage.value;");        
	out.println("url = \"http://"+connect.hostname+"/servlets/Form3?master=\"+master+\"&menufacture=\"+menufacture+\"&model=\"+model+\"&year=\"+year+\"&sp_namex=\"+sp_namex+\"&lastpage=\"+lastpage;");
	out.println("if (year!=\"\")"); 
	out.println("window.location.href = url;");
	out.println("}");
...
out.println("<TABLE bgColor=#FF80FF border=2 borderColor=#0000ff borderColorDark=#0000ff borderColorLight=#0000ff cellPadding=2 cellSpacing=0 width=500>");		
	...
	out.println("<TD width=\"60%\"><FONT face=Arial size=2>&nbsp;&nbsp;");
      ...
      pstmt = con.prepareStatement("SELECT distinct car_menufacturer FROM car ORDER BY car_menufacturer ASC");  
      rs = pstmt.executeQuery();

	out.println("<SELECT NAME=\"menufacture\" SIZE=\"1\" onchange=\"doReload1(this.form)\">");
	out.println("<OPTION SELECTED VALUE=\"\">---------------</OPTION>");

      while(rs.next()) {
        out.println("<OPTION VALUE=\"" + rs.getObject("car_menufacturer").toString() + "\">" + rs.getObject("car_menufacturer").toString() + "</OPTION>" );
	} 
  	out.println("</SELECT>");  
...
//------------------------2---------------------------------    
      ...
      pstmt = con.prepareStatement("SELECT distinct model FROM car ORDER BY model ASC");
      rs = pstmt.executeQuery();

	out.println("<SELECT NAME=\"model\" SIZE=\"1\" onchange=\"doReload2(this.form)\">");
	out.println("<OPTION SELECTED VALUE=\"\">----------------------</OPTION>");
...
//------------------------3---------------------------------    
      ...
      pstmt = con.prepareStatement("SELECT distinct year FROM car ORDER BY year ASC");
      rs = pstmt.executeQuery();

	out.println("<SELECT NAME=\"year\" SIZE=\"1\" onchange=\"doReload3(this.form)\">");
	out.println("<OPTION SELECTED VALUE=\"\">-------</OPTION>");

      while(rs.next()) {
        out.println("<OPTION VALUE=\"" + rs.getObject("year").toString() + "\">" + rs.getObject("year").toString() + "</OPTION>" );
      } 
      out.println("</SELECT>");
