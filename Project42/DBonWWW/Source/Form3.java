...
public class Form3 extends HttpServlet {
  ...
  public void service(HttpServletRequest req, HttpServletResponse res)
   ...
   if(values!=null) 
	...
	out.println("function doReload31(entry) {");
	...
	out.println("url = \"http://"+connect.hostname+"/servlets/Form12?master=\"+master+\"&menufacture=\"+menufacture+\"&model=\"+model+\"&year=\"+year+\"&sp_namex=\"+sp_namex+\"&lastpage=\"+lastpage;");
	out.println("if (menufacture!=\"\")"); 
	out.println("window.location.href = url;");
	out.println("}");

	out.println("function doReload32(entry) {");
	out.println("var current1,current2,current3;");		
	out.println("var menufacture,model,year,master,url;");
	out.println("current1 = entry.menufacture.selectedIndex;");
	out.println("menufacture = entry.menufacture.options[current1].value;");
	...
	out.println("year = entry.year.options[current3].value;");
        ...
        out.println("sp_namex=entry.sp_namex.value;");        
        out.println("lastpage=entry.lastpage.value;");        
	out.println("url = \"http://"+connect.hostname+"/servlets/Form32?master=\"+master+\"&menufacture=\"+menufacture+\"&model=\"+model+\"&year=\"+year+\"&sp_namex=\"+sp_namex+\"&lastpage=\"+lastpage;");
...
//------------------------1---------------------------------
      ...
      pstmt = con.prepareStatement("SELECT distinct car_menufacturer FROM car WHERE year="+"'"+year+"' ORDER BY car_menufacturer ASC");  
      rs = pstmt.executeQuery();

	out.println("<SELECT NAME=\"menufacture\" SIZE=\"1\" onchange=\"doReload31(this.form)\">");
...
//------------------------2---------------------------------    
      ...
      pstmt = con.prepareStatement("SELECT distinct model FROM car WHERE year="+"'"+year+"' ORDER BY model ASC");
      rs = pstmt.executeQuery();

	out.println("<SELECT NAME=\"model\" SIZE=\"1\" onchange=\"doReload32(this.form)\">");
	out.println("<OPTION SELECTED VALUE=\"\">----------------------</OPTION>");
