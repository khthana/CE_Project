...
Dim Receivedata(3) As Byte
Dim Receiveflag As Boolean
Dim Codedata(3) As Byte
Dim datanum As Integer
Dim continue As Boolean
Dim enterkey As String
Dim delaynum As Long
Dim Havecomm As Boolean
...
'On Error GoTo abc
    ...
    If foundhl = False Then     ' not found
        Unload Me
        MsgBox ("Hardlock Not Found")
    End If
Exit Sub
abc:
    MsgBox ("Can't Access Communication Port")
    Unload Me
End Sub
...
Sub Codecheck()
    ...
    End If
        ...
        Codedata(0) = Int(Rnd * 255)
        Codedata(1) = Int(Rnd * 255)
        Codedata(2) = Int(Rnd * 255)
        Codedata(3) = Int(Rnd * 255)

        Senddata(0) = Codedata(0)
        Senddata(1) = Codedata(1)
        Senddata(2) = Codedata(2)
        Senddata(3) = Codedata(3)
        ...
        If Receivedata(0) = Codedata(0) And Receivedata(1) = Codedata(1) And Receivedata(2) = Codedata(2) And Receivedata(3) = Codedata(3) Then
            foundhl = True
        Else
            foundhl = False
        End If
