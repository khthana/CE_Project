...
$date = `date`;
extract ($date);
$nummonth = $monthno;			#real day number in table
...
for ($i = 0; $i < $countread1; $i++) {
	...
	if ($query->param("$compare[0]") =~ /^on$/i) {
		...
		$checkmax	= $compare[4];
		$checkmin	= $compare[5];
		$checktable	= $compare[3].'_y';
		
		$filename = $checkip.'.yrs';	#name of file for graph
		...
		if ($checkoid == 1) {	#inoctet and outoctet
			$sth = $dbh->prepare("select inf,mon,inoctavg,outoctavg from $checktable
				where mon <= $nummonth and inf=0");
			...
			$sth = $dbh->prepare("select inf,mon,inerravg,outerravg from $checktable
				where mon <= $nummonth");
		...
		if (!system($RTMT_DIR."html/gd/create5", "$checkip", "$checkmax",
			...
			print "<a href=\"/rtmt/cgi-bin/yearly.pl?ip=$checkip&oid=$checkoid&type=$checktype\">";
			print "$checkip</h1>\n";
			print "<image src=\"/rtmt/gd/image/year/$checkip"."_0.png\">";
			print "</a></p>\n";
		} else {
			print "$checkip</h1>\n";
			print "ERROR: \n";
