...
public class defertranpdays extends HttpServlet{
...
////////////////////////////////////////
	...
	while (rs2.next())
	{
		if (rs2.getString("inter").equals(ip+"#2") ){
			...
			int tempinoctet1 = Integer.parseInt(rs2.getString("defertran1").trim());	
			vinoctet1.addElement(new Integer(tempinoctet1));

			if (rs2.getString("defertran2").equals("e")){
				...
				while (rs8.next())
				{
					if (rs8.getString("inter").equals(ip+"#2") ){
						int inoctetcur  = Integer.parseInt(rs8.getString("defertran").trim());
				...
				int tempinoctet2 = Integer.parseInt(rs2.getString("defertran2").trim());	
...
//				mi=mi+1;  //add				
    ...
    try {
      ...
      chart.getBackground().setTitleString("Deferred Transmissions Statistics (per Day)");
      ...
      chart.getDatasets()[0].getGc().setFillColor(Color.blue);
...
bb:	while (rs88.next())
	{
		if (rs88.getString("inter").equals(ip+"#2") && 
			...
			wanted = Integer.parseInt(rs88.getString("defertran").trim());	
			break bb;
		}
	}
