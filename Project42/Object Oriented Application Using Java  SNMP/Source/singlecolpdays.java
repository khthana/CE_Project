...
public class singlecolpdays extends HttpServlet{
...
////////////////////////////////////////
	...
	while (rs2.next())
	{
		if (rs2.getString("inter").equals(ip+"#2") ){
			...
			int tempinoctet1 = Integer.parseInt(rs2.getString("singlecol1").trim());	
			vinoctet1.addElement(new Integer(tempinoctet1));

			if (rs2.getString("singlecol2").equals("e")){
				...
				while (rs8.next())
				{
					if (rs8.getString("inter").equals(ip+"#2") ){
						int inoctetcur  = Integer.parseInt(rs8.getString("singlecol").trim());
				...
				int tempinoctet2 = Integer.parseInt(rs2.getString("singlecol2").trim());	
...
//				mi=mi+1;  //add				
    ...
    try {
      ...
      chart.getBackground().setTitleString("SingleCollisionFrames Statistics(per Day)");
      ...
      chart.getDatasets()[0].getGc().setFillColor(Color.green);
...
bb:	while (rs88.next())
	{
		if (rs88.getString("inter").equals(ip+"#2") && 
			...
			wanted = Integer.parseInt(rs88.getString("singlecol").trim());	
			break bb;
		}
	}
