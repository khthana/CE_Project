...
public class datao1pday extends HttpServlet{
...
////////////////////////////////////////
	...
	while (rs2.next())
	{
		if (rs2.getString("inter").equals(ip+"#1") ){
			...
			int tempinoctet1 = Integer.parseInt(rs2.getString("outoctet1").trim());	
			...
			if (rs2.getString("outoctet2").equals("e")){
				...
				while (rs8.next())
				{
					if (rs8.getString("inter").equals(ip+"#1") ){
						int inoctetcur  = Integer.parseInt(rs8.getString("outoctet").trim());
				...
				int tempinoctet2 = Integer.parseInt(rs2.getString("outoctet2").trim());	
...
//				mi=mi+1;  //add				
    ...
    try {
      ...
      chart.getBackground().setTitleString("OutOctet in Interface1 Statistics (per day)");
      ...
      chart.getDatasets()[0].getGc().setFillColor(Color.orange);
...
bb:	while (rs88.next())
	{
		if (rs88.getString("inter").equals(ip+"#1") && 
