...
uses
  Windows, Classes, MMSystem, SysUtils;
...
constructor TProtectEXE.Create(AOwner : TComponent);
begin
...
function TProtectEXE.CheckExe(Report : Boolean) : TExeStatus;
begin
...
procedure TProtectEXE.DoOnChecked(Status : TExeStatus);
begin
  if Assigned(FOnChecked) then
    FOnChecked(Self, Status);
end;

procedure TProtectEXE.Loaded;
begin
  inherited Loaded;
...
function TProtectEXE.StampExe(const FileName : string ;  EraseMarker : Boolean) : Boolean;
begin
  Result := ProtectExe(FileName,  EraseMarker);
end;

function TProtectEXE.UnStampExe(const FileName : string) : Boolean;
begin
